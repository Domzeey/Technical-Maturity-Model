...
function Questionnaire() {
  ...
  const handleNext = () => {
    if (responses[questions[currentQuestion - 1].id]) {
  ...
  const handlePrevious = () => {
    if (currentQuestion > 1) {
      setCurrentQuestion((prev) => prev - 1);
      setError(''); // Clear error message when moving back
    }
  };
  ...
  return (
    <div className="background">
      ...
      <div className="container mt-4">
        ...
        <p><strong>Question {currentQuestion} of {totalQuestions}</strong></p>

        {/* Current Question and Options */}
        <div className="card mb-3">
          <div className="card-body">
            <h5>{questions[currentQuestion - 1].question}</h5>
        ...
        <div className="d-flex justify-content-between">
          <Button variant="secondary" onClick={handlePrevious} disabled={currentQuestion === 1}>Previous</Button>
          <Button variant="primary" onClick={handleNext}>{currentQuestion === totalQuestions ? 'Submit' : 'Next'}</Button>
        </div>
      </div>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          <h5>Your Score: {totalScore} / {maxScore}</h5>
          <h5>Percentage Score: {percentageScore}%</h5>
          <h5>Maturity Level: {maturityLevel}</h5>
